...
def controllo(operatore, operazione):
    ...
    db.read_in_db(cur, "select * from utenti;")
    ...
    if operatore["user"] in users:
        ...
        ind = users.index(operatore["user"])

        if operatore["password"] == utenti[ind][1]:
            #carico i permessi
            permesso = utenti[ind][2]
